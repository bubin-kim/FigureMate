...
- **🎉 MVP 파이프라인 5단계 전체 완결 (2026-07-14).** 백엔드 테스트 78개 통과.
- ✅ post-MVP 개선 완료 (2026-07-14~15): Vision 모델 qwen2.5vl:7b 승격(실측 근거는
  architecture.md 2.4), 한국어 설명 + 문체 게이트(합쇼체 표준 — `pipeline._clean_style`),
  느슨한 캡션 규칙(Springer 스타일, A+B 검증), 홈 화면 논문 히스토리(`GET /papers`).
- 🚧 다음: M6 이후 — `docs/roadmap.md`의 방향성 항목에서 사용자가 우선순위를 정하면
  **새 마일스톤 문서를 먼저 작성하고 승인 후 착수한다.** (배포는 사용자가 보류로 결정, 2026-07-15)
...
- git 저장소이며 원격은 **github.com/bubin-kim/FigureMate (공개)**. 푸시 전 키 스캔 습관 유지
  (.env·storage·샘플 PDF는 .gitignore로 제외됨).
...
```
frontend (Next.js)       ✅ 뷰어 UI (M5 — 업로드/히스토리/Figure 목록/상세/본문 하이라이트)
   │ REST
backend/app/api          ✅ FastAPI 라우트 (papers, figures — 목록·업로드·조회·매칭·이미지)
backend/app/graph        ✅ LangGraph 파이프라인 (parse→extract→match→explain→finalize)
backend/app/agents       ✅ 개별 Agent (노드). 폴더 1개 = Agent 1개 (grounding/explainer)
